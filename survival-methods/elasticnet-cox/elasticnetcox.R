...
nr.of.iterations.to.perform <- 3
# nr.of.iterations.to.perform <- 50
...
# Pdf for the plots
pdf('~/Documents/msc-beatriz-r-correia/survival-methods/elasticnet-cox/results/dev_alpha0.1_test.pdf')
# -------------------------------------------------------------------
  ...
  for (k in 1:length(measures.list)) {
    ...
    for (i in 1:length(alpha.values.list)) {
      tryCatch({
        ...
        names(output.info) <- c("measure", "alpha", "nr.selected.genes", "list.of.genes", "pvalue.train", "pvalue.test", "iteration.nr", "cindex.train", "cindex.test")
        ...
        output.info[1] <- measures.list[k]
        output.info[2] <- alpha.values.list[i]
        list.of.fits[[fit.name]] <- cv.glmnet(xtrain, Surv(ytrain$time, ytrain$status),
                                              family  = 'cox',
                                              nlambda = 100,
                                              nfolds = 10,
                                              type.measure = measures.list[k],
