...
# Pdf for the plots
pdf('~/Documents/msc-beatriz-r-correia/survival-methods/orphancox/results/alpha0.1_test.pdf')
# -------------------------------------------------------------------
  ...
  for (k in 1:length(measures.list)) {
    ...
    for (i in 1:length(alpha.values.list)) {
      tryCatch({
        ...
        list.of.fits[[fit.name]] <- cv.glmOrphan(xtrain, Surv(ytrain$time, ytrain$status),
                                              family  = 'cox',
                                              nlambda = 100,
                                              nfolds = 10,
                                              alpha = alpha.values.list[i],
                                              network = "correlation",
                                              network.options = networkOptions(cutoff = 0.001, min.degree = min.degree.v))
